...
def parse_msid(msid_directory):
    ...
    temp_msid = ascii.read(msid_directory + "2CE00ATM.csv", format="fast_csv")
    pitch_msid = ascii.read(msid_directory + "Point_SunCentAng.csv", format="fast_csv")
    dist_msid = ascii.read(msid_directory + "Dist_SatEarth.csv", format="fast_csv")
...
def plot_temps(data):
    ...
    scattertimes = data["Times"] / 365.2422

    plot = ax.scatter(scattertimes, data["CEA Temperature"], c=data["Pitch"], cmap='RdBu', vmin=0, vmax=180, label="Maximum CEA Temperature (2CE00ATM)")
